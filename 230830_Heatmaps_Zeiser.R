...
metadata_filter <- metadata_tib %>% filter(grepl("AML.*D0", orig.ident) | grepl("BM", orig.ident)) %>%
...
# P-values
plot_mat[,1:3]
t.test(x = plot_mat["HAVCR2",grepl("BM", colnames(plot_mat))], y = plot_mat["HAVCR2",!grepl("BM", colnames(plot_mat))])
t.test(x = plot_mat["LGALS9",grepl("BM", colnames(plot_mat))], y = plot_mat["LGALS9",!grepl("BM", colnames(plot_mat))])
t.test(x = plot_mat["HMGB1",grepl("BM", colnames(plot_mat))], y = plot_mat["HMGB1",!grepl("BM", colnames(plot_mat))])
# Out of curiosity
as_tibble(t(plot_mat), rownames = "cell") %>% left_join(metadata_filter) %>% group_by(CellType_Donor) %>% summarize(mean_TIM3 = mean(HAVCR2)) %>% print(n = Inf)